...
Player game sprites: Annie Zhou, created with Procreate using IPad
Cover image = "Cook Kitchen Chef Cooker Cooking Icon" by photo3idea
Chef hat: Annie Zhou, created with Procreate using IPad
Octopus hat: Annie Zhou, created with AdobeXD using Macbook
Stem hat: Annie Zhou, created with AdobeXD using Macbook
Straw hat: "Beach Hat of Man" from grmarc 
Santa Hat: "santas hat" from Roundicons Pro
Lose image: "Man Showing Loser with Sign Fingers Head" by MaryValery
Winner: "Winner with Trophy" by liliakyrylenko
Cookbook: open book by DAPA images
Whisk: Bowl whisk by vectorwin
Plate: Plate by icons8
Saute = "Saute Cooking Pan" by smallike
Oven = "Oven" by smallike
Blender = "Isolated Blender" by iconsy
Isometric kitchen appliances = https://image.freepik.com/free-vector/kitchen-equipment-isometric-set_1284-24576.jpg
Isometric kitchen = https://assets.materialup.com/uploads/2298c5ea-1ba3-488d-9cc1-6dba8d0dd755/preview.jpg
OpenCookbook = "Recipe book line and fill style" by justicon
Title screen, instructions screen, basket screen, kitchen screen, appliance screen, judging screen, login screen, customize screen, winner screen, loser screen: Annie Zhou, created with Procreate using IPad (linked here: https://www.canva.com/design/DAEPj0yjwCE/share/preview?token=HatXrdlkyxuTlAzVlZEKZA&role=EDITOR&utm_content=DAEPj0yjwCE&utm_campaign=designshare&utm_medium=link&utm_source=sharebutton_

Ingredient icons: https://icons8.com/icon/pack/food/color

Game AI sprites: https://image.slidesharecdn.com/ww2rtsgameinhtml5-131015125425-phpapp02/95/html5-bug-25d-rts-game-in-html5-by-dawid-lijewski-22-638.jpg?cb=1381841723
"""
...
class CookingMode(Mode):
    ...
    def appStarted(self):
        ...
        self.skill = 0
        self.combinationScreenPath = './images/combinationScreen.png'
        ...
        self.cookbookScreen = './images/cookbookScreen.png'
        self.imgcookbookScreen = self.loadImage(self.cookbookScreen)
        self.isExitCombination = False 
        self.youMadePath = './images/youMade.png'
        self.imgYouMade = self.loadImage(self.youMadePath)
        self.combinationImg = self.loadImage('./images/cookedRice.png')
        self.timerDelay = 200
...
#CITATION: CODE MATH FROM http://clintbellanger.net/articles/isometric_math/ 
    ...
    def setUpIsometric(self):
        ...
        self.blenderPath = './images/blender.png'
    ...
    def setUpCharImages(self):    
        self.imgPath = './images/icons8-cake-96.png'
        if self.isCasual:
            self.kitchenPath = './images/casualScreen.png'
        else:
            self.kitchenPath = './images/competitionScreen.png'
        #self.img = self.loadImage(self.imgPath)
        self.imgKitchen = self.loadImage(self.kitchenPath)
        self.leftPath = './images/left.png'
        self.rightPath = './images/right.png'
        self.downPath = './images/down.png'
        self.upPath = './images/up.png'
        ...
        self.hatPaths = {'chef': './images/chef.png', 
        ...
        img = self.loadImage('./images/AIUp.png')
        scaleFactor = CookingMode.findScaleFactor(self, img, self.width//6)

        self.gameAIDir = self.scaleImage(self.loadImage('./images/AIUp.png'), scaleFactor)
         
    #set up cooking Screens for images
    def setUpCooking(self):
        self.blendImg = self.loadImage('./images/cookingscreen/blend.png')
        self.bakeImg = self.loadImage('./images/cookingscreen/bake.png')
        self.sauteImg = self.loadImage('./images/cookingscreen/saute.png')
        self.stackImg = self.loadImage('./images/cookingscreen/stack.png')
        self.mixImg = self.loadImage('./images/cookingscreen/mix.png')
            ...
            if not self.moveNum == len(self.moveList): 
                ...
                if not self.moveList == None:
                    ...
                    if currMove == (0, 1) or currMove == (1, -1) or currMove == (-1, 0):
                        ...
                        self.gameAIDir = self.scaleImage(self.loadImage('./images/AIRight.png'), scaleFactor)
                    elif currMove == (0, -1):
                        self.gameAIDir = self.scaleImage(self.loadImage('./images/AILeft.png'), scaleFactor)
                    elif currMove == (1, 0):
                        self.gameAIDir = self.scaleImage(self.loadImage('./images/AIDown.png'), scaleFactor)
                    elif currMove == (-1, -1):
                        self.gameAIDir = self.scaleImage(self.loadImage('./images/AIdiagonalLeft.png'), scaleFactor)
                    elif currMove == (1, 1):
                        self.gameAIDir = self.scaleImage(self.loadImage('./images/AIdiagonalRight.png'), scaleFactor)
                    else:
                        self.gameAIDir = self.scaleImage(self.loadImage('./images/AIUp.png'), scaleFactor)
                    if currMove[0] == 20:
                        self.waitTime = 500
...
class JudgingMode(Mode):
    def appStarted(self):
        ...
        self.judgingPath = './images/judgingScreen.png'
        self.judgingImg = self.loadImage(self.judgingPath)
        self.losePath = './images/cookingscreen (6)/lose.png'
        self.loseImg = self.loadImage(self.losePath)
        self.winPath = './images/cookingscreen (6)/win.png'
    ...
    def drawPlayerInfo(self, canvas):
        ...
        if self.isPlayerCalculated:
            ...
            canvas.create_text(self.width/4, 4*self.height/5, text=f'Webscraping Score: {self.playerWebScore}', font = 'Verdana 14 bold')
    ...
    def redrawAll(self, canvas):
        ...
        if self.isGameAICalculated and self.isPlayerCalculated:
            r = self.height//3
            canvas.create_text(self.width/2, self.height-20, text = 'SCORES TALLIED! PRESS "w" TO MOVE ON!', fill = 'black')
...
class LeaderboardMode(Mode):
    def appStarted(self):
        ...
        self.imgCombinationScreen = self.loadImage(self.combinationScreenPath)
    def modeActivated(self):
        LeaderboardMode.appStarted(self)
    def keyPressed(self, event):
...
class ShoppingMode(Mode):
    def appStarted(self):
        ...
        self.currentSelect = list() 
        self.hand = list() 
        shoppingScreenPath = './images/shoppingscreen.png'  
        self.shoppingScreen = self.loadImage(shoppingScreenPath)
        shelfPath = './images/shelf.png'
        self.shelf = self.loadImage(shelfPath)
    def modeActivated(self):
        ShoppingMode.appStarted(self)
...
class TitleMode(Mode):
    def appStarted(self):
        self.cookingScreen = self.loadImage('./images/cookingScreen.png')
        #post-mvp functionality of a hard vs a easy mode
        self.isHard = False
...
class BasketMode(Mode):
    def appStarted(self):
        ...
        #post mvp functionality of taking customized dietary list into account
        try: 
...
class LogInMode(Mode):
    def appStarted(self):
        self.loginScreenPath = './images/loginPage.png'
...
class CustomizeMode(Mode):
    def appStarted(self):
        self.combinationScreenPath = './images/cookingscreen (8)/customize.png'
        self.imgCombinationScreen = self.loadImage(self.combinationScreenPath)
        self.user = self.app.logInMode.user
        self.charPath = './images/down.png'
    ...
    def redrawAll(self, canvas):
        ...
        canvas.create_text(self.width/2, self.height/3.9, font = 'Verdana 20 bold', text=f'{self.user}'.upper(), fill = 'white')
        if self.isExisting:
            canvas.create_text(self.width/10, self.height/3, font = 'Verdana 13 bold', text=f'Recipes made: {self.recipes}', fill = 'white', anchor='w')
...
class InstructionsMode(Mode):
    ...
    def appStarted(self):
        ...
        if not self.isCasual:
            ...
            self.instructionsPath = './images/instructions.png'
        else:
            self.instructionsPath = './images/casualmode.png'
    ...
    def setUpInstructions(self, instructionsPath):
        ...
        if isCasual:
            instructionsPath = './images/instructions.png'
        else:
            instructionsPath = './images/casualmode.png'
